...
// startReconnect 开始重连 （暂时用不着）
func (cm *ConnectionManager) waitReconnect() {
	defer cm.wg.Done()
	...
	for i := 1; i <= cm.config.MaxReconnectTries; i++ {
		...
		llog.Infof("[lagrange.连接] 尝试重连 (%d/%d)", i, cm.config.MaxReconnectTries)
